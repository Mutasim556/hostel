...
use App\Models\Admin\SeatServiceType;
...
class SeatController extends Controller
{
    ...
    public function store(Request $data)
    {
        ...
        foreach($data->service_type as $key=>$value){
            $seatService = new SeatServiceType();
            $seatService->room_id = $data->room;
            $seatService->seat_id = $seat->id;
            $seatService->service_type_id = $value;
            $seatService->charge = $data->type_charge[$key];
            $seatService->save();
        }
        return [
    ...
    public function show(string $id)
    {
        ...
        if($data[1]=='hostel'){
            ...
            $rooms = Room::where([['hostel_id',$data[2]],['building_id',$data[3]],['floor',$data[4]]])->select('id','room_number','block','room_type')->get();
            return [
                'rooms'=>$rooms,
            ];
        }elseif($data[1]=='room'){
    ...
    public function getServiceType(string $type){
        $service = ServiceType::where([['status',1],['delete',0],['room_type',$type]])->get();
        // dd($service);
        return $service;
    }

    public function getServiceTypeCharge(string $id ,string $days , string $rooms){
        $service = ServiceType::where([['status',1],['delete',0],['id',$id]])->first();

        return $service->charge*$rooms*$days;
    }
}

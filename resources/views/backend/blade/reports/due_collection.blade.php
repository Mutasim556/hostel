 ...
 @push('title')
     {{ __('admin_local.Due Collection Report') }}
 @endpush
 @push('css')
             ...
             <div class="col-lg-12 mx-auto">
                 <div class="card">
                     <div class="card-header py-3" style="border-bottom: 2px dashed gray">
                         <h3 class="card-title mb-0 text-center">{{ __('admin_local.Due Collection Report') }}</h3>
                     </div>

                     <div class="card-body">
                         <div class="row">
                             <div class="col-md-12">
                                 <form action="{{ route('admin.reports.dueCollection') }}" id="filter_form" method="GET">
                                     <div class="row bg-info p-4 mb-4 text-center">
                                         ...
                                         <div class="col-md-12 mt-3" {{--  style="border:1px solid red;"  --}}>
                                             ...
                                             <a href="{{ route('admin.reports.dueCollection') }}"
                             ...
                             <table id="basics-1" class="display table-bordered" style="width: 100%">
                                 <thead>
                                     <tr>
                                         ...
                                         <th>{{ __('admin_local.Pay Amount') }}</th>
                                     </tr>
                                 </thead>
                                 <tbody>
                                     @foreach ($bookingPayments as $payment)
                                         <tr id="trid-{{ $payment->id }}" data-id="{{ $payment->id }}">
                                             <td>{{ '#' . sprintf('%08u', $payment->invoice->id) }}</td>
                                             <td>{{ $payment->invoice->bookingperson->booking_phone_number }}</td>
                                             <td>{{ $payment->invoice->bookingperson->booking_person_name }}</td>
     ...
     <script>
         ...
         var oTable = $("#basics-1").DataTable({
             ...
             buttons: [{
                 ...
                 },
                 {
                     ...
                     customize: function(xlsx) {
                         ...
                         var newRow =
                             ...
                             '<c t="inlineStr" r="E' + rowIndex + '" s="2"><is><t>Total:</t></is></c>' +

                             // Total value in 6th column
                             '<c t="n" r="F' + rowIndex + '"><v>' + seatTotalAll.toFixed(2) + '</v></c>' +
                             '</row>';
                 ...
                 // },
                 {
                     ...
                     customize: function(doc) {
                         ...
                         footerRow[5].text = seatTotalAll.toFixed(2);
             ...
             footerCallback: function(row, data, start, end, display) {
                 ...
                 // Total calculations
                 var collectionSum = api.column(5, {
                     page: 'current'
                 }).data().reduce(function(a, b) {
                     return parseValue(a) + parseValue(b);
                 }, 0);
                 ...
                 // Update footer cells
                 $(api.column(5).footer()).html(collectionSum.toFixed(2));
             }
         });

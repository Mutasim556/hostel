        ...
        <div class="details">
            <table>
                <tr>
                    ...
                    <td>#{{ str_pad($bookingI->id, 8, '0', STR_PAD_LEFT) }} [
                        {{ date('Y-m-d h:i:s A', strtotime($bookingI->created_at)) }} ]</td>
                </tr>
                <tr>
                ...
                @php
                    $bookingData = \App\Models\Admin\Booking::with('hostel', 'room')
                        ->where('id', $bookingI->bookings->first()->id)
                        ->first();
                    // dd($bookingData);
                @endphp
                <tr>
                ...
                <tr>
                    ...
                    <td>{{ $bookingI->service->service_name . ' [ ' . $bookingI->service->service_type . '-' . $bookingI->service->service_code . ' ]' }}
                    </td>
                </tr>
                service
                <tr>
                    ...
                    <td>
                        @foreach ($bookingI->bookings as $key => $value)
                            @php
                ...
                <tr>
                    ...
                    <td>{{ date('d M , Y', strtotime($bookingI->booking_start_date)) }} -
                        {{ date('d M , Y', strtotime($bookingI->booking_end_date)) }}</td>
                </tr>
                <tr>
                    <td class="label">Total Fee:</td>
                    <td> BDT {{ $bookingI->seat_price }} + BDT {{ $bookingI->seat_service_charge }} [ Service Charge ]
                    </td>
                </tr>
                <tr>
